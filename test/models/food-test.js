        ...
        .then(function() {
          Food.all()
            .then(function(data) {
              ...
              assert.equal(data.rows[0].name, 'cheeseburger')
              assert.equal(data.rows[0].active, true)
              assert.equal(data.rows[0].calories, 800)
              assert.ok(data.rows[0].created_at)
              assert.ok(data.rows[0].updated_at)
              assert.equal(data.rows[1].id, 1)
              assert.equal(data.rows[1].name, 'pizza')
              assert.equal(data.rows[1].active, true)
              assert.equal(data.rows[1].calories, 1000)
